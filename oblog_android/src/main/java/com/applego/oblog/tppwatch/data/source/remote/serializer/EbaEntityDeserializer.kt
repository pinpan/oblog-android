...
class EbaEntityDeserializer : JsonDeserializer<EbaEntity> {
    override fun deserialize(json: JsonElement?, typeOfT: Type?, context: JsonDeserializationContext?): EbaEntity? {

        var jsonObject: JsonObject? = json?.asJsonObject
        val ebaEntity = convertFrom(jsonObject)
        return ebaEntity
    }
    ...
    companion object {
        private var INSTANCE: EbaEntityDeserializer? = null
        val tppDeserializer: EbaEntityDeserializer
            get() {
                if (INSTANCE == null) {
                    INSTANCE = EbaEntityDeserializer()
                }
                return INSTANCE!!
            }
    }

    fun convertFrom(jsonObject: JsonObject?) : EbaEntity? {
        if (jsonObject == null) {
            return null
        }
        ...
        val ebaEntity = EbaEntity(_entityId = entityId, _entityCode = entityCode, _entityName = entityName, _description = description, _globalUrn = globalUrn, _ebaEntityVersion = ebaEntityVersion, _country = country, _entityType = EbaEntityType.findByCode(entityType))
        ...
        try {
            if (services != null) {
                ...
                ebaEntity._ebaPassport = passportedServices
            }
        } catch (t: Throwable ) {
            Timber.e(t)
        }
